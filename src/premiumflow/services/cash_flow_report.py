...
from .cash_flow_pnl_aggregators import ZERO
...
def _parse_stock_lot_closed_date(value: Optional[str]) -> Optional[date]:
    """Parse the `closed_at` timestamp stored for stock lots into a date."""
    if not value:
        return None
    normalized = value
...
def _aggregate_stock_realized_by_period(
    ...
    *,
    ...
    """Aggregate realized stock P&L grouped by closing period."""
    ...
    for lot in stock_lots:
        ...
        if closed_date is None or not _date_in_range(closed_date, since, until):
            continue
...
def _create_empty_report(
    ...
    """Return an empty cash flow report (no periods)."""
...
def _filter_transactions_by_date(
    ...
    *,
    ...
    """Filter transactions by the requested date range."""
...
def generate_cash_flow_pnl_report(  # noqa: PLR0913
    ...
    *,
    ...
    """
    Generate the cash flow and P&L report with period-based grouping.
    """
    params = _CashFlowPnlReportParams(
        ticker=ticker,
        since=since,
