...
def test_fetch_stock_transactions_filters(tmp_path, repository):
    ...
    hoood_rows = repository.fetch_stock_transactions(ticker="HOOD")
    assert len(hoood_rows) == 2

    dated = repository.fetch_stock_transactions(ticker="HOOD", since=date(2025, 9, 2))
    assert len(dated) == 1
